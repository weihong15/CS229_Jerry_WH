...
def init_centroids(num_clusters, image):
    ...
    for i in range(num_clusters):
        centroids_init.append(image[random.randint(0,h-1),random.randint(0,w-1),:])
...
def update_centroids(centroids, image, max_iter=30, print_every=10):
    ...
    for iter in range(max_iter):
        ## First group every point to nearest group
        #for pixel in ...
        if iter%print_every == 0:
            print("iter:{} out of {}".format(iter,max_iter))

        numMembersInCentroid = np.zeros(len(centroids))
        sumRGBInCentroid = np.zeros((len(centroids),c))
        for x in range(h):
            for y in range(w):
                # note each pixel is x,y
                pixel = image[x,y,:]
                
                dist = np.zeros(len(centroids))
                for ind in range(len(centroids)):
                    dist[ind]= np.linalg.norm(centroids[ind,:]-pixel)

                numMembersInCentroid[dist.argmin()] +=1
                sumRGBInCentroid[dist.argmin(),:] += pixel

    ## find the centroid of each new group
        for i in range(len(centroids)):
            if numMembersInCentroid[i]>0:
                centroids[i,:] = sumRGBInCentroid[i,:]/numMembersInCentroid[i]
...
def update_image(image, centroids):
    ...
    image : nparray
        (H, W, C) image represented as an nparray
    centroids : int (definitely not int, should be (numOfCentroid * c))
        The centroids stored as an nparray
    ...
    # *** START YOUR CODE ***
    h,w,c = image.shape
    ## loop through all pixel, see which centroid nearer then assign
    ...
    for x in range(h):
        for y in range(w):
            ...
            for ind,centre in enumerate(centroids):
                dist = np.linalg.norm(centre-pixel)
                if dist<bestDist:
                    bestDist = dist
                    nearest = ind

            image[x,y,:] = centroids[nearest,:]
       # *** END YOUR CODE ***
