...
def main(is_semi_supervised, trial_num):
    ...
    for i in range(K):
        if (i!=K-1):
            group = x[i*numInGroup:(i+1)*numInGroup]
        else:
            group = x[i*numInGroup:]
        mu.append(np.mean(group,axis=0))
        sigma.append(np.cov(group.T,bias=True))
...
def gaussian(x,mu,sigma):
    ...
    num = np.dot(x-mu,np.linalg.inv(sigma))@(x-mu)
    num = np.exp(-0.5*num)
    den = (2*np.pi)**(x.shape[0]/2)*np.sqrt(np.linalg.det(sigma))
    return num/den

def gaussianArr(X,mu,sigma):
    ans = np.zeros(X.shape[0])
    for i in range(X.shape[0]):
        ans[i] = gaussian(X[i],mu,sigma)
    return ans
    ...
    while it < max_iter and (prev_ll is None or np.abs(ll - prev_ll) >= eps):
        ...
        for i in range(n):
            for j in range(K):
                ...
                w[i,j] = phi[j] *gaussian(x[i],mu[j],sigma[j])
        ##normalized each row
        ...
        for j in range(K):
            p_x_given_z =  gaussianArr(x, mu[j], sigma[j])
...
def run_semi_supervised_em(x, x_tilde, z_tilde, w, phi, mu, sigma):
    ...
    while it < max_iter and (prev_ll is None or np.abs(ll - prev_ll) >= eps):
        ...
        phi = phi / (n+alpha*n_tilde)
        for j in range(K):
            mu[j] = (np.dot(w[:,j],x)+alpha*np.dot(w_tilde[:,j],x_tilde))/(np.sum(w[:,j])+alpha*np.sum(w_tilde[:,j]))
            sigma[j] = (x - mu[j]).T.dot(np.diag(w[:, j])).dot(x - mu[j]) \
                +alpha * (x_tilde - mu[j]).T.dot(np.diag(w_tilde[:, j])).dot(x_tilde - mu[j])
            sigma[j] /= (w[:, j].sum()+alpha*w_tilde[:, j].sum())
        ...
        for j in range(K):
            p_x_given_z =  gaussianArr(x, mu[j], sigma[j])
            p_x += p_x_given_z * phi[j] 
        p_x_tilde = np.zeros(n_tilde)
        for j in range(K):
            p_x_tilde +=phi[j]* gaussianArr( x_tilde , mu[j], sigma[j])
        ll = np.sum(np.log(p_x)) +alpha*np.sum(np.log(p_x_tilde))
        # print("semi supervised iteration : {}, ll = {}".format(it,ll))
        it += 1
    print("semi supervised iteration : {}, ll = {}".format(it,ll))
