...
def choose_action(state, mdp_data):
    ...
    # *** START CODE HERE ***
    ## select starting state and 1 action
...
def update_mdp_transition_probs_reward(mdp_data):
    ...
    # *** START CODE HERE ***
    ## first count number of times sa --> s'
    total = mdp_data['transition_counts'].sum(axis=1) ##shape (s,a), sum up s' 
    for s in range(total.shape[0]):
        for a in range(2):
            if total[s,a] != 0:
                mdp_data['transition_probs'][s,:,a] = mdp_data['transition_counts'][s,:,a]/total[s,a]

    for i in range(mdp_data["reward"].shape[0]):
        total = mdp_data["reward_counts"][i,1]
        if total !=0:
            ## rewards count [0], how many -ve value, [1] total value
            mdp_data["reward"][i] = -mdp_data["reward_counts"][i,0]/total
    # *** END CODE HERE ***
...
def update_mdp_value(mdp_data, tolerance, gamma):
    ...
    # *** START CODE HERE ***
    iter = 0
    ## run value iteration here
    ## new value = reward at this state me gamma best action sum Psa(s')*V(s')
    #oldValue = mdp_data['value']
    newValue = None
    while (newValue is None or np.max(np.abs(newValue-oldValue))>tolerance):
        oldValue = mdp_data['value']
        action0 = mdp_data['transition_probs'][:,:,0]@oldValue
        action1 = mdp_data['transition_probs'][:,:,1]@oldValue
        bestaction = np.maximum(action0,action1)
        newValue = mdp_data['reward'] + gamma * bestaction
        mdp_data['value'] = newValue
        iter +=1
    return iter==1
    # *** END CODE HERE ***
